...
def get_detect_language() -> List[str] :
    retval = ["en-US"]
    language = get_cmd_option("--detectLanguages")
    if language is not None :
        retval = language.split(",")
    return retval

def get_target_language() -> List[str] :
    retval = ["en"]
    language = get_cmd_option("--targetLanguages")
    if language is not None:
        retval = language.split(",")
    return retval
    ...
    return helper.Read_Only_Dict(
        {
            ...
            "detect_languages": get_detect_language(),
            "target_languages": get_target_language(),
